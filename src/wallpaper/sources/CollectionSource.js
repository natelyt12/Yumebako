...
import { addToCollection, getCollection, getCollectionItem } from "/src/wallpaper/sources/api/collectionDb.js";
 ...
 *
 * Its trailing "+" item is the way wallpapers enter the collection: it advertises
...
export class CollectionSource extends BaseSource {
    ...
    /** The picker only opens from a real click on the "+" item. */
     ...
     * Called from a real click on the "+" item, i.e. inside a user gesture.
    ...
    async fetchMore() {
        ...
        try {
            ...
            // and the "+" item must stay available for another try.
            ...
            return added.length ? added.map((raw) => this._toCarouselItem(raw)) : null;
        } finally {
    ...
    isActive(item) {
        ...
        return (
            ...
            (config.activeCollectionItemId === item.id || config.activeWallpaperId === item.id)
        );
    }

    async apply(item, { firstRun = false } = {}) {
        const raw = await getCollectionItem(item.id);
        const isVideo = raw.type === "local_video" || raw.type === "video" || (raw.blob instanceof Blob && raw.blob.type.startsWith("video/"));
        await wallpaperRenderer.apply({ ...raw, id: item.id, type: isVideo ? "video" : "image" }, { firstRun });
    }
