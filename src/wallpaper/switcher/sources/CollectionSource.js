...
import { addToCollection, getCollection, getCollectionItem } from "/src/wallpaper/providers/impl/collection/collectionDb.js";
 ...
 *
 * Its trailing "+" card is the way wallpapers enter the collection: it advertises
 * an upload that the user has to click. It cannot grow on its own — browsers
 * only open a file picker inside a user gesture — hence `moreNeedsGesture`.
 */
export class CollectionSource extends BaseSource {
    ...
    get actions() {
        return [SOURCE_ACTIONS.download, SOURCE_ACTIONS.source, SOURCE_ACTIONS.remove];
    }
    ...
    get moreLabel() {
        return t("wallpaper_switcher.click_to_upload", "Ấn để tải lên");
    }

    /** The picker only opens from a real click on the "+" card. */
     ...
     * Called from a real click on the "+" card, i.e. inside a user gesture.
     * @returns {Promise<Array<Object>>} The cards that were just added.
     */
    async fetchMore() {
        if (this.isLoading) return null;
        this.isLoading = true;
        try {
            // A cancelled picker, or files that all failed: nothing was produced
            // and the "+" card must stay available for another try.
            const files = await pickMediaFiles();
            if (files.length === 0) return null;

            const added = [];
            for (const file of files) {
                const record = await this._addLocalFile(file);
                if (record) added.push(record);
            }
            return added.length ? added.map((raw) => this._toCard(raw)) : null;
        } finally {
    ...
    /**
     * Compress and store one picked file.
     * @returns {Promise<Object|null>} The stored collection record, if it made it in.
     */
    async _addLocalFile(file) {
        ...
        try {
            ...
            const { width, height } = isVideo ? { width: 0, height: 0 } : await getImageDimensions(file);

            const record = await addToCollection({
                type: isVideo ? "local_video" : "local_image",
                blob: file,
                thumbnail,
                metadata: { width, height, size: file.size, source: "local", mimeType: file.type },
            });

            showNotification(t("sp.api.collection.upload_success", { count: 1 }), "success");
            return record;
        } catch (error) {
            console.error(`[Collection] Failed to process "${file.name}":`, error);
            showNotification(t("sp.api.collection.upload_error", { file: file.name }), "error");
            return null;
        }
    }

    /**
     * Every collection entry becomes a card whose media stays in the app.
     *
     * The window is rebuilt from scratch, but its object URLs are not. The cards
    ...
    async fetchCards() {
        ...
        const cards = collection.map((raw) => this._toCard(raw));

        this._releaseUnusedThumbs(new Set(cards.map((card) => card.id)));
        return cards;
    }
    ...
    async prepareThumb(card) {
        ...
        card.thumbnailUrl = this._resolveThumb(await getCollectionItem(card.id));
    ...
    isActive(card) {
        ...
        return (
            ...
            (config.activeCollectionItemId === card.id || config.activeWallpaperId === card.id)
        );
    }

    async apply(card, { firstRun = false } = {}) {
        const raw = await getCollectionItem(card.id);
        if (!raw?.blob) return;
        await providerManager.applyCollectionItem(raw, firstRun);
    }
    ...
    /** Map one collection record onto the persisted card shape. */
    _toCard(raw) {
        const metadata = raw?.metadata || {};
        const sourceUrl = /^https?:\/\//i.test(metadata.source || "") ? metadata.source : "";
    ...
    _buildTitle(raw) {
        ...
        const mediaType = t(`sp.api.collection.${isVideo ? "typeVideo" : "typeImage"}`, isVideo ? "Video" : "Ảnh");
        ...
        if (providerName && providerKey !== "local") {
            srcVal = providerName;
        } else if (rawSource === "local" || providerKey === "local" || raw.type?.startsWith("local")) {
