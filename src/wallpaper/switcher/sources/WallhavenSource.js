...
import { fetchWallhavenQueue } from "/src/wallpaper/providers/impl/wallhaven/wallhavenAPI.js";
 ...
 * `wallhaven_data` queue; the carousel then materializes them one at a time as
 * the user reaches the trailing "+" card, refilling the queue when it runs dry.
 *
 * An unreachable API raises instead of yielding an empty batch, so the card can
...
export class WallhavenSource extends RemoteSource {
    ...
    static storageKey = "wallhaven_data";
