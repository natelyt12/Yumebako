...
import { getSettings, saveSettings, getDefaultWallpaperEffects } from "/src/core/storageHandler.js";
import { showNotification } from "/src/core/ui.js";
import { createSources } from "./sources/index.js";
import { CARD_STATE, isMoreCard } from "./stores/cardSchema.js";
import { SwitcherStore } from "./stores/SwitcherStore.js";
import { migrateCardStores } from "./stores/migration.js";
import { SourceTabs } from "./components/SourceTabs.js";
import { SourceActions } from "./components/SourceActions.js";
import { CarouselTrack } from "./components/CarouselTrack.js";

import switcherHtml from "./switcher.html?raw";
import "./switcher.css";
...
export class WallpaperSwitcher {
    ...
    async init() {
        ...
        this.carouselEl = this.container.querySelector(".wallpaper_carousel_container");
        ...
        try {
            const report = await migrateCardStores();
    ...
    get activeSource() {
        return this.sources.find((source) => source.id === this.activeSourceId) || this.sources[0];
    }

    /** Card window of a source, created on first use. */
    getStore(sourceId) {
        if (!this.stores.has(sourceId)) {
            const source = this.sources.find((entry) => entry.id === sourceId) || this.activeSource;
    ...
    async loadSource(sourceId, { refresh = false } = {}) {
        const available = this.availableSources;
        const source = available.find((item) => item.id === sourceId) || available[0] || this.sources[0];
        ...
        try {
            ...
            console.error(`[WallpaperSwitcher] Failed to load source [${source.id}]:`, error);
            showNotification(error?.message || t("sp.api.error", { provider: source.name }), "error");
        }
    ...
    _renderCarousel(store, index = store.index) {
        const cards = store.moreState === CARD_STATE.END ? store.items : store.cards;
    ...
    async growSource() {
        ...
        const moreIndex = this.carousel.selectedIsMore ? this.carousel.selectedIndex : -1;
        ...
        store.setMoreState(CARD_STATE.LOADING);
        this._syncMore(store);
        ...
        try {
            ...
            if (added === null) {
                store.setMoreState(CARD_STATE.IDLE);
                this._syncMore(store);
                return;
            }
            ...
            if (!added.length) {
                store.setMoreState(CARD_STATE.END);
            ...
            const following = moreIndex !== -1 && this.carousel.selectedIndex === moreIndex;

            // Compute dropped cards if store trimmed oldest items
            const newStoreCardIds = new Set(store.cards.map((c) => c.id));
            const droppedIds = [];
            this.carousel.cards.forEach((c) => {
                if (!isMoreCard(c) && !newStoreCardIds.has(c.id)) {
            ...
            await this.carousel.animateInsertCard(added[0], { following, droppedIds });
            ...
            console.error(`[WallpaperSwitcher] Failed to grow source [${source.id}]:`, error);
            showNotification(error?.message || t("sp.api.error", { provider: source.name }), "error");
            store.setMoreState(CARD_STATE.ERROR);
            this._syncMore(store, t("wallpaper_switcher.retry_hint", "Lỗi — bấm để thử lại"));
    ...
    _startGrowCooldown(source) {
        ...
        this._cooldownTimer = setInterval(() => this._tickGrowCooldown(), COOLDOWN_TICK_MS);
    }
    ...
    _tickGrowCooldown() {
        ...
        if (remaining > 0) {
            this._syncMore(store, t("wallpaper_switcher.cooldown", { seconds: Math.ceil(remaining / 1000) }));
            return;
        }
    ...
    async recoverThumbnail(card) {
        const source = this.sources.find((entry) => entry.id === card?.source) || this.activeSource;
    ...
    async selectSource(sourceId) {
        ...
        saveSettings({ wallpaperSwitcher: { ...settings.wallpaperSwitcher, activeSource: sourceId } });
    ...
    syncWallpapersStorage(items) {
        const settings = getSettings();
        const storedWallpapers = Array.isArray(settings.wallpapers) ? [...settings.wallpapers] : [];
        let updated = false;
    ...
    async applyCurrentWallpaper({ force = false, firstRun = false } = {}) {
        ...
        if (!card || isMoreCard(card)) return;
        ...
        try {
            ...
            await store.remember({ index: this.carousel.selectedIndex, appliedId: card.id });
        } catch (error) {
    ...
    async removeCardAnimated(card, neighbour) {
        const store = this.getStore(this.activeSourceId);
        const wasActive = this.activeSource.isActive(card);

        // If the card being removed is the active desktop wallpaper,
        // start applying neighbour immediately so desktop transitions concurrently!
        if (wasActive && neighbour) {
            this.activeSource
                .apply(neighbour)
                .then(() => store.setApplied(neighbour.id))
                .catch((err) => {
                    console.error("[WallpaperSwitcher] Error applying neighbour wallpaper:", err);
                });
        }
